...
const struct key_name_entry KEY_NAMES[] = {{"SPACE", KEY_SPACE},
					   {"ENTER", KEY_ENTER},
					   {"ESC", KEY_ESC},
					   {"TAB", KEY_TAB},
...
enum desktop_env detect_desktop_env(void)
{
	...
	if (env_contains("XDG_CURRENT_DESKTOP", "KDE"))
		return DE_KDE;
	if (env_contains("XDG_CURRENT_DESKTOP", "GNOME"))
		return DE_GNOME;
...
const char *de_name(enum desktop_env de)
{
	...
	case DE_GNOME:
		return "GNOME";
	case DE_XFCE_X11:
		return "XFCE-family / X11-keys";
	case DE_SWAY:
		return "Sway";
	case DE_HYPRLAND:
		return "Hyprland";
	default:
		return "unknown (defaulting to X11-keys)";
	}
}
