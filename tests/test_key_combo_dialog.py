"""Tests for the key-combo parse/format helpers.

The dialog itself needs a Qt event loop and is exercised by hand; the
parser is what the daemon wire format ultimately depends on, and it
lives in the Qt-free key_combo module so the pytest job runs without
PySide6 installed.
"""

from spacemouse_config import key_combo


def test_parse_typical_combo():
    assert key_combo.parse_combo_string("Ctrl+Shift+S") == (["Ctrl", "Shift"], "S")


def test_parse_plain_key_no_modifier():
    # Mirrors "key:F" in the daemon wire format — zero modifiers is valid.
    assert key_combo.parse_combo_string("F") == ([], "F")
...
def test_parse_is_case_insensitive_and_canonicalises_key():
    ...
    assert key_combo.parse_combo_string("ctrl+shift+s") == (["Ctrl", "Shift"], "S")
    assert key_combo.parse_combo_string("CONTROL+ALT+F1") == (["Ctrl", "Alt"], "F1")
    assert key_combo.parse_combo_string("ctrl+TAB") == (["Ctrl"], "Tab")
    assert key_combo.parse_combo_string("Meta+pageup") == (["Meta"], "PageUp")
...
def test_parse_modifier_aliases():
    ...
    for alias in ("Super", "Win", "Cmd", "Meta"):
        mods, key = key_combo.parse_combo_string(f"{alias}+L")
...
def test_format_canonical_modifier_order():
    ...
    # diff-wise.
    assert key_combo.format_combo(["Shift", "Ctrl"], "S") == "Ctrl+Shift+S"
    assert key_combo.format_combo(["Meta", "Alt", "Ctrl"], "Tab") == "Ctrl+Alt+Meta+Tab"


def test_format_empty_key_returns_empty():
    # The dialog uses this to detect "user hasn't picked anything yet".
    assert key_combo.format_combo(["Ctrl"], "") == ""
    assert key_combo.format_combo([], "") == ""
...
def test_format_drops_unknown_modifiers():
    ...
    assert key_combo.format_combo(["Ctrl", "Bogus"], "S") == "Ctrl+S"
...
def test_round_trip_property():
    ...
    for canonical in cases:
        mods, key = key_combo.parse_combo_string(canonical)
        assert key_combo.format_combo(mods, key) == canonical, (
